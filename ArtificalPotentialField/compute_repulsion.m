function [Yrerxx,Yreryy,Yataxx,Yatayy]=compute_repulsion(X,Xsum,m,angle_at,angle_re,n,Po,a)%输入参数X为当前坐标，Xsum是目标和障碍的坐标向量，m增益常数,n障碍数，angle_at车辆与目标方向的角度，angle_re车辆与障碍物的角度
...
for i=1:n
    ...
    R0=(Xsum(1,1)-Xsum(i+1,1))^2+(Xsum(1,2)-Xsum(i+1,2))^2;%目标点与障碍物之间的距离平方
    r0=sqrt(R0);%目标点与障碍物之间的距离保存在数组r0中
    if rre(i)>Po%如果每个障碍和路径点的距离大于障碍影响距离，令斥力为0
        Yrerx(i)=0;
        Yrery(i)=0;
        Yatax(i)=0;
        Yatay(i)=0;
