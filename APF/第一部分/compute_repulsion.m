function [Yrerxx,Yreryy,Yataxx,Yatayy]=compute_repulsion(X,Xsum,m,angle_at,angle_re,n,Po,a)%输入参数为当前坐标，Xsum是目标和障碍的坐标向量，增益常数,障碍，目标方向的角度
    ...
    for i=1:n
        ...
        R0=(Xsum(1,1)-Xsum(i+1,1))^2+(Xsum(1,2)-Xsum(i+1,2))^2;%目标点和障碍物的距离平方
        r0=sqrt(R0);%目标点和障碍的距离保存在数组r0中
        if rre(i)>Po || rat<15%如果每个障碍和路径的距离大于障碍影响距离或者距离目标点的距离小于阈值，斥力令为0
            Yrerx(i)=0;
            Yrery(i)=0;
            Yatax(i)=0;
            Yatay(i)=0;
            ...
            if rre(i)<Po/2
                ...
                %这里对原程序按照改进的方案进行了简单的修改，这部分对照记的笔记部分需要对分解的两个向量进行理解
                Yrer(i)=m*(1/rre(i)-1/Po)*(1/Rrei(i))^2*(rat^a);%分解的Fre1向量,斥力方向
                Yata(i)=a*m*((1/rre(i)-1/Po)^2)*(rat^(a-1))/2;%分解的Fre2向量,引力方向
